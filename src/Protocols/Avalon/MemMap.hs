...
deriving instance (KnownManagerConfig config)
...
deriving instance (KnownSubordinateConfig config)
...
deriving instance (KnownSharedConfig config,
...
mmSubordinateInNoData :: (KnownSubordinateConfig config) =>
  AvalonSubordinateIn config
    ...
    stateFn so dfAck dfDat = do
      ...
      put
        ( if ( Maybe.isNothing readDatStored1 -- to avoid looking at dfAck when not needed
            || dfAck) then Nothing else readDatStored1
        , readReqAcked1)
...
instance (KnownManagerConfig config, config ~ RemoveNonDfManager config) =>
