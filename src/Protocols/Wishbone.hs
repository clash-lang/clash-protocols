...
instance (C.ShowX dat, C.KnownNat addressWidth, C.KnownNat selWidth)
    => Show (WishboneM2S addressWidth selWidth dat) where
  show = C.showX
...
pattern Classic
      ...
      , IncrementingBurst
      , EndOfBurst :: CycleTypeIdentifier
...
instance Protocol (Wishbone dom mode addressWidth dat) where
  type Fwd (Wishbone dom mode addressWidth dat) =
...
roundrobin
  ...
  . ( C.KnownNat n, C.HiddenClockResetEnable dom, C.KnownNat addressWidth
    , C.KnownNat (C.BitSize a), C.NFDataX a, 1 C.<= n )
...
sharedBus
  ...
  . ( C.KnownNat n, C.KnownNat m, C.HiddenClockResetEnable dom
    , C.KnownNat addressWidth, C.KnownNat (C.BitSize a), C.NFDataX a )
  => (C.Signal dom ( C.Index n
                   , C.Index m
...
crossbarSwitch
  ...
  . ( C.KnownNat n, C.KnownNat m, C.KnownDomain dom, C.KnownNat addressWidth
    , C.NFDataX a, C.KnownNat (C.BitSize a) )
  => Circuit
