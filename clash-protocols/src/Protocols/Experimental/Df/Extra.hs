...
{- | A buffer that can store a single element and will acknowledge data if it can store
it. Also indicates when it is ready to accept data. In practice, the ready signal is
...
This buffer is implemented in terms of 'moore', so it cuts up combinational paths.

This buffer inserts bubbles every other cycle under full input throughput.
-}
singleEntryBuffer ::
  forall dom a.
  (NFDataX a, HiddenClockResetEnable dom) =>
  Circuit (Df dom a) (Df dom a, CSignal dom Ready)
singleEntryBuffer = Circuit go
 where
  go (dataIn, (ackIn, _)) = (ackOut, (dataOut, readyOut))
   where
    (ackOut, dataOut, readyOut) = mooreB goState goOutput InReset (dataIn, ackIn)

  goState :: BufferState a -> (Maybe a, Ack) -> BufferState a
  ...
  goOutput :: BufferState a -> (Ack, Maybe a, Bool)
  goOutput s = let ack = isEmpty s in (Ack ack, toMaybe s, ack)

  isEmpty :: BufferState a -> Bool
  isEmpty Empty = True
  isEmpty _ = False

  toMaybe :: BufferState a -> Maybe a
