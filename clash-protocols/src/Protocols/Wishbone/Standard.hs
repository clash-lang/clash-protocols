...
memoryWb ram = Circuit go
 where
  ...
  fsm st m2s
    ...
    | not masterActive = (Wait, (0, Nothing, noS2M))
    | isError = (Wait, (0, Nothing, noS2M{err = True}))
    -- write requests can be ACKed directly
    | Wait <- st, isWrite = (Wait, (0, write, noS2M{acknowledge = True}))
    -- read ACKs need to be delayed one cycle
    | Wait <- st, isRead = (AckRead, (addr m2s, Nothing, noS2M))
    | AckRead <- st = (Wait, (0, Nothing, noS2M{acknowledge = True}))
    | otherwise = (Wait, (0, Nothing, noS2M))
   where
    noS2M = emptyWishboneS2M @()
    masterActive = busCycle m2s && strobe m2s
    isError = masterActive && (busSelect m2s /= maxBound)
    isWrite = masterActive && writeEnable m2s
    isRead = masterActive && not (writeEnable m2s)
    write = Just (addr m2s, writeData m2s)
